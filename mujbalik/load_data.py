...
def load_json_data(file_path):
    ...
    Args:
        file_path: cesta k .json souboru, který chci přečíst (třeba "data/planets.json")

    Returns:
        data: vrací data, která se nacházela v souboru, jako Python slovník
    """
    # pouzijeme 'with open', aby se soubor hned po otevření sám zavřel
    with open(file_path, 'r') as file:
        data = json.load(file)      # převedení z .json souboru na Python slovník
    return data
...
def convert_json_to_csv(json_data, output_csv):
    ...
    Args:
        json_data: slovnik s informacemi o planetách,
        output_csv: cesta k .csv souboru, kde chci uložit data
    ...
    for planet_name, planet_info in json_data.items():
        ...
        pos = planet_info['position']
        velocity = planet_info['velocity']
    ...
    df.to_csv(output_csv, index=False)          # ondex=False znamená, že nechceme mít cisla v CSV souboru
    print(f"Data úspěsně převedena do souboru: {output_csv}")


def save_scenario_to_csv(names, positions, velocities, masses, filename="scenario_data.csv"):
    """
    Uloží data vygenerovaného scénáře (pozice, rychlosti, hmotnosti) do CSV souboru.

    Args:
        names: Seznam názvů těles
        positions: Numpy pole s pozicemi těles
        velocities: Numpy pole s rychlostmi těles
        masses: Numpy pole s hmotnostmi těles
        filename: Název souboru pro uložení dat (výchozí je "scenario_data.csv")
    ...
    if not (len(names) == positions.shape[0] == velocities.shape[0] == masses.shape[0]):
        raise ValueError("Počet prvků v seznamech/polích (názvy, pozice, rychlosti, hmotnosti) se neshoduje.")
